...
function _omb_theme_PROMPT_COMMAND() {
  ...
  if [ -n "$SCHROOT_ALIAS_NAME" ]; then
    schroot_env="${_omb_prompt_teal}[${SCHROOT_ALIAS_NAME}]"
  fi
  ...
  PS1="$schroot_env$python_venv${hostname} ${_omb_prompt_bold_teal}\W $(scm_prompt_char_info)${ret_status}→ ${_omb_prompt_normal}"
}
